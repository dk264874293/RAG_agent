...
chunker = AdaptiveChunker(
    {
        ...
        "use_llama_index_semantic": settings.use_llama_index_semantic,
        "doc_type_mapping": settings.doc_type_mapping,
    }
)
...
async def readiness_check():
    ...
    if is_ready:
        return JSONResponse(
            content={"status": "ready", "checks": checks}, status_code=200
        )
    else:
        return JSONResponse(
            content={"status": "not_ready", "checks": checks}, status_code=503
        )
...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        documents = await document_processor.process_document(
            file_path, metadata={"filename": file.filename, "upload_time": time.time()}
        )
        ...
        for doc in documents:
            chunks = chunker.chunk_document(
                doc.page_content, doc_type=doc.metadata.get("file_type", "default")
            )
            # 为每个块创建Document对象
...
async def query(request: QueryRequest, user_request: Request):
    ...
    try:
        ...
        context_docs = await quality_optimizer.optimize_retrieval(
            filtered_query, hybrid_retriever
        )
